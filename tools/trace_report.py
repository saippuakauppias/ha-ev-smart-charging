...
def anomalies(runs: list[dict[str, Any]], tz: dt.timezone) -> list[str]:
    ...
    for run in runs:
        ...
        #
        # Замеры при выключенной зарядке отбрасываются по той же причине:
        # ток спадает не мгновенно, и мгновение остановки даёт «недодачу»
        # в две трети уставки. В пятую ночь один такой замер (14 А против
        # 4.8 А в 06:56) поднял среднее с 1.25 до 1.52 А.
        if (isinstance(setpoint, (int, float)) and isinstance(actual, (int, float))
                and setpoint > 0 and actual > 0.5
                and values.get("switch_on") is not False):
            shortfalls.append((setpoint - actual) / setpoint)
    ...
    # за смещение станции.
    #
    # Меряем долю, а не амперы, потому что «Запас по току» — множитель,
    # и сравнивать с ним абсолютную величину нельзя. По амперам совет выходил
    # заведомо неверным: в пятую ночь инструмент написал «недодаёт 1.52 А,
    # поднимите запас» при запасе 10 %, который эту недодачу уже покрывал.
    #
    # Сам разрыв при этом постоянен именно в амперах (см. комментарий
    # к current_shortfall в блюпринте), так что доля верна только для того
    # диапазона токов, в котором прошла разобранная сессия. Отсюда
    # осторожность формулировки: сравнить с настройкой, а не «поднять».
    if shortfalls:
        average = sum(shortfalls) / len(shortfalls) * 100
        if average > 3:
            found.append(
                f"станция отдавала на {average:.0f} % меньше уставки "
                f"(по {len(shortfalls)} замерам) — сверьте с «Запасом по току»; "
                f"на других токах доля будет иной, разрыв в амперах постоянен"
            )
    return found
